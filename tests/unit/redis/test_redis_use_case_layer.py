"""Unit tests for cache use cases."""
...
async def test_set_and_get_cache_use_cases() -> None:
    ...
    get_result = await GetCacheEntryUseCase(repository).execute(GetCacheEntryQuery(key="alpha"))
...
async def test_delete_cache_use_case() -> None:
    ...
    result = await DeleteCacheEntryUseCase(repository).execute(DeleteCacheEntryCommand(key="alpha"))
